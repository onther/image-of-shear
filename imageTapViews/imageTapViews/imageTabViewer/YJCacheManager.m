...
#import "YJCacheManager.h"

@interface YJCacheManager()
{
...
@implementation YJCacheManager

+(instancetype)shareCacheManager{
    return self.manager;
}
+(YJCacheManager *)cacheManagerWithidentifier:(NSString *)identifier{
    return [[YJCacheManager alloc]initWithIdentifier:identifier];
}
+(YJCacheManager *)manager{
    static YJCacheManager *shareInstance =nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken ,^{
        shareInstance = [[YJCacheManager alloc]initWithIdentifier:NSStringFromClass(self)];
    });
    return shareInstance;
}

-(id)initWithIdentifier:(NSString *)isdntifier{
    if (isdntifier.length <= 0) {
...
#pragma mack --directory operation
    ...
    for(int i=0; i<16; i++) {
        for(int j=0; j<16; j++) {
            ...
            for(id name in list) {
                ...
                if(attr) {
                [files addObject:[[YJFileAttribute alloc] initWithPath:filePath attribues:attr]];
                }
            }
        }
    }
